...
const deleteAllWastes = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `${result.deletedCount}  deletetion successfully` });
  } catch (error) {
...
const markAsRead = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: "Les collections ont été marquées comme lues avec succès.",
      });
  } catch (error) {
    ...
    return res
      ...
      .json({
        message:
          "Une erreur est survenue lors de la mise à jour des collections.",
      });
  }
};
module.exports = {
  ...
  updateCollectionByshipper,
  getShippedCollection,
  updateCollectionByCenter,
};
