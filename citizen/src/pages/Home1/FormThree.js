  ...
  const {id,WalletID} = useContext(AuthContext);
    ...
    const addToBin = async (binID, citizenID, weight,BlockchainID,citizenWalletID) => {
      ...
      const finish = async () => {
        ...
        try {
            const result = await addToBin(updateContext.binID, id, updateContext.quantity, updateContext.BlockchainID, WalletID);            
    ...
    return (
      ...
      <Toast ref={toast} />
        <div className="container-home">
            ...
            <button className="multipleButton" value="Done" type="button" onClick={finish}>Done</button>
            </div>
        </div>
      </div>
    );
